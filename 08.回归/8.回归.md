...
![平方误差](https://i.imgur.com/tUweKo1.png)

用矩阵表示还可以写做 ![平方误差_2](https://i.imgur.com/fVj1aoQ.png) 。如果对 w 求导，得到 ![平方误差_3](https://i.imgur.com/UyuzOHJ.png) ，令其等于零，解出 w 如下（具体求导过程为: http://blog.csdn.net/nomadlx53/article/details/50849941 ）:

![回归系数的最佳估计计算公式](https://i.imgur.com/pnbUw6E.png)
...
![回归系数的最佳估计计算公式](/https://i.imgur.com/8E0ldJR.png)
...
![线性回归数据示例图](https://i.imgur.com/fCncDds.png "线性回归数据示例图")
...
![线性回归数据效果图](https://i.imgur.com/GYZR8gO.png "线性回归数据效果图")
...
![参数k与权重的关系](https://i.imgur.com/Jd8LkVN.png)
...
![局部加权线性回归数据示例图](https://i.imgur.com/SukIEab.png)
...
![局部加权线性回归数据效果图](https://i.imgur.com/qVamaTS.png)
...
如果数据的特征比样本点还多应该怎么办？是否还可以使用线性回归和之前的方法来做预测？答案是否定的，即我们不能再使用前面介绍的方法。这是因为在计算 ![矩阵求逆](https://i.imgur.com/Z7aZfzf.png) 的时候会出错。
...
简单来说，岭回归就是在矩阵 ![矩阵_1](https://i.imgur.com/jzumbb4.png) 上加一个 λI 从而使得矩阵非奇异，进而能对 ![矩阵_2](https://i.imgur.com/gSw3Ewv.png ) 求逆。其中矩阵I是一个 n * n （等于列数） 的单位矩阵，
对角线上元素全为1，其他元素全为0。而λ是一个用户定义的数值，后面会做介绍。在这种情况下，回归系数的计算公式将变成：

![岭回归的回归系数计算](https://i.imgur.com/hfTAl17.png )

岭回归最先用来处理特征数多于样本数的情况，现在也用于在估计中加入偏差，从而得到更好的估计。这里通过引入 λ 来限制了所有 w 之和，通过引入该惩罚项，能够减少不重要的参数，这个技术在统计学中也叫作 `缩减(shrinkage)`。

![岭回归](https://i.imgur.com/2WoJiLw.png)
...
![岭回归的运行效果](https://i.imgur.com/MX9iXth.png)
...
![逐步线性回归运行效果](https://i.imgur.com/mKQki1q.png)
...
![生成公式](https://i.imgur.com/zlxjF3M.png)
...
![偏差方差图](https://i.imgur.com/PujIMHP.png)
...
* * *
* **参考信息来源于 [ApacheCN](http://www.apachecn.org/)**
