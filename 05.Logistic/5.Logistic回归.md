...
![Sigmoid 函数计算公式](https://i.imgur.com/Z3zk06y.png)
...
![Sigmoid 函数在不同坐标下的图片](https://i.imgur.com/V4GmJgX.png)
...
![Sigmoid 函数计算公式](https://i.imgur.com/GF0Fakp.png)

如果采用向量的写法，上述公式可以写成 ![Sigmoid 函数计算公式向量形式](https://i.imgur.com/E6oxSyz.png) ，它表示将这两个数值向量对应元素相乘然后全部加起来即得到 z 值。其中的向量 x 是分类器的输入数据，向量 w 也就是我们要找到的最佳参数（系数），从而使得分类器尽可能地精确。为了寻找该最佳参数，需要用到最优化理论的一些知识。我们这里使用的是——梯度上升法（Gradient Ascent）。
...
![梯度上升计算公式](https://i.imgur.com/99oLYwk.png)

这个梯度意味着要沿 x 的方向移动 ![f(x, y)对x求偏导](https://i.imgur.com/uhn0GRq.png) ，沿 y 的方向移动 ![f(x, y)对y求偏导](https://i.imgur.com/oLfLknL.png) 。其中，函数f(x, y) 必须要在待计算的点上有定义并且可微。下图是一个具体的例子。

![梯度上升](https://i.imgur.com/ivSI8xV.png)
...
![梯度上升迭代公式](https://i.imgur.com/3Wilks3.png)
...
![梯度下降迭代公式](https://i.imgur.com/KopaQiH.png)


**局部最优现象 （Local Optima）**

![梯度下降图_4](https://i.imgur.com/FY2rC4x.png)
...
![方向导数与梯度](https://i.imgur.com/M7SO3MW.png)
...
![简单数据集绘制在图上](https://i.imgur.com/SU7Mk7c.png)
...
![f(w)对w求一阶导数](https://i.imgur.com/Z3O9wSd.png)
...
![回归系数与迭代次数的关系图](https://i.imgur.com/u4tBRVa.png)
...
![改进随机梯度下降结果图](https://i.imgur.com/1WPSl54.png)
